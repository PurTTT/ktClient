...
//	@Update("update room set totalRequest = totalRequest + 1 where roomNum = (#{roomNum})")
//	int addtotalR(@Param("roomNum") Integer roomNum);
	...
	@Select("SELECT roomId, MAX(openTime) as openTime, SUM(windTime) as totalTime, SUM(fee) as totalFee, COUNT(roomId) as BillNum, MAX(tempRequest) as tempRequest, MAX(windRequest) as windRequest FROM BillList NATURAL JOIN roomList WHERE date_format(endTime,'%Y-%m-%d')<= (#{endTime}) and date_format(beginTime,'%Y-%m-%d') >= (#{beginTime}) GROUP BY roomId")
